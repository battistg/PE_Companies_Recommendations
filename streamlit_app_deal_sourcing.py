...
**Instructions:**
- In the **Prediction Input** tab, enter the details of a new company and click **"Classify Company"** to receive the prediction.
- In the **Model Insights** tab, view interactive visualizations such as feature importance and SHAP explanations that help understand the model's decision process.
...
if menu == "Prediction Input":
    ...
    if st.button("Classify Company"):
        ...
        input_data = {
            ...
            # Categorical values to be encoded later:
            'country': country,
            'sector': sector
        }
        ...
        probability = model.predict_proba(input_df)[0][1]  # Assuming True means High Potential
...
# Model Insights Page
    ...
    st.write("### Explore Model Insights and Explanations")
    ...
    # Create a DataFrame for feature importances using the model's attribute
    ...
    st.write("The SHAP values below help explain the contribution of each feature to the model's prediction for a sample input.")
    
    # Initialize SHAP
    shap.initjs()
    
    # Create a sample input from dummy data (for illustration)
    # In a production environment, you should use a representative sample of the training data.
    sample_data = {col: 1.0 for col in feature_columns}  # dummy value for each feature
    sample_df = pd.DataFrame([sample_data])
    
    # Compute SHAP values using TreeExplainer (for Random Forest)
    explainer = shap.TreeExplainer(model)
    shap_values = explainer.shap_values(sample_df)
    
    st.write("SHAP Summary Plot (Bar Plot):")
    # Plot SHAP summary as a bar plot (using matplotlib)
    fig_shap, ax = plt.subplots(figsize=(8, 4))
    shap.summary_plot(shap_values[1], sample_df, plot_type="bar", show=False)
    st.pyplot(fig_shap)
    
    st.write("Note: The above SHAP plot is generated using a dummy sample. For a more accurate explanation, use a representative sample from your training data.")
